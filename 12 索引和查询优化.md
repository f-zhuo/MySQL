...
索引本质是**排好序**的快速**查找**的数据结构，本身很大，以索引文件的形式存储在磁盘上。MySQL会另外维护一个查找算法的数据结构来支持索引，一般都为B树/B+树，还有哈希，R树，fulltext。以B树为例，假设索引是从1排到100，现在查找索引为65的记录。在B树的最顶端为索引50，65比50大，就向右查找；第二层有索引25和75，65比75小，就向左查找；第三层从75分出了60和90，65比60大，再向右查找，依次就可找到索引为65的记录。这样的数据结构避免了从1遍历到65，提高了执行效率

![E:\jupyter notebook\mysql1\pictures\B树结构.png](E:\jupyter notebook\mysql1\pictures\B树结构.png)
